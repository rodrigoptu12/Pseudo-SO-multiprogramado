...
std::vector<int> ProcessManager::executedProcesses;
...
void ProcessManager::loadInitFile(const char *processFileName)
{
  ...
  std::istringstream process_file(sortByTimeStamp(fileContent)); // convert content to stream to read line by line
  std::string line;
...
void ProcessManager::deleteProcess(int pid)
{
  ...
  for (long unsigned int i = 0; i < processes.size(); i++)
  {
    ...
    {
      process = processes[i];
      executedProcesses.push_back(processes[i]->getPID());
...
{
  // Verifica se o processo existe no array de executados
  for (long unsigned int i = 0; i < executedProcesses.size(); i++)
  {
    if (executedProcesses[i] == pid)
      return true;
  }
  return false;
}
...
void ProcessManager::allocateResourcesToProcessesInQueues()
{
  ...
  if (!Queues::isPrinter1QueueEmpty() && Resources::getAvailablePrinter(1))
  {
    ...
    if (process->hasAllocatedResources())
      Queues::addToReadyQueue(process);
  }
...
bool ProcessManager::verifyPrintersResource(Process *process)
{
  ...
  { // process does not require printer
    return true;
  }
  else if (!process->getPrinterAllocated())
  { // printer is not allocated
...
{
  if (!process->getIsScannerUsed())
  { // process does not require scanner
    return true;
  }
  else if (!process->getScannerAllocated())
  { // scanner is not allocated
...
{
  if (process->getDiskNumber() == 0)
  { // process does not require driver
    return true;
  }
  else if (!process->getDriverAllocated())
...
{
  if (!process->getIsModemUsed())
  { // process does not require modem
    return true;
  }
  else if (!process->getModemAllocated())
  { // modem is not allocated
